...
const getStatusConfig = (status: string) => {
  ...
  if (s === 'APPROVED' || s === 'PAID') return statusConfig.PAID;
  if (s === 'CANCELLED' || s === 'REJECTED' || s === 'FAILED') return statusConfig.REJECTED;
              ...
              <div className="p-12 flex flex-col md:flex-row justify-between gap-10">
                <div className="flex-1">
                  <div className="flex items-center gap-5 mb-8">
                    <span className={`flex items-center gap-2 px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border ${config.color}`}>
                      <span className={`w-2 h-2 rounded-full animate-pulse ${config.signal}`} />
